...
&& in_array($extension, $allowedExts))
{
    ...
    else
    {
       ...
       // echo "文件临时存储的位置: " . $_FILES["upfile"]["tmp_name"] . "<br>";
        ...
        if (file_exists("../image/" . $_FILES["upfile"]["name"]))
        {
        ...
        else
        {
            ...
            echo "文件存储在: " . "../image/" . $_FILES["upfile"]["name"];
        }
    }
}
else
...
$sql="SELECT *FROM news order by id desc limit 1";//找出最后一条记录的id
...
$sql1="insert into yancey.news(id,title,new,picture,time,author,djcs,shzt) values('$id','$title','$new','$picture','$time','$author','$djcs','$sh')";
$result=mysqli_query($con,$sql1);
//echo $sql1;
echo "<script>alert('数据成功添加！');location.href='design.php';</script>";
